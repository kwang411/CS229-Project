...
def setup_sampler(space, scores, k, alpha, gamma):
    sp = np.array(space)
    d0 = np.max(sp[:,0]) - np.min(sp[:,0])
    d1 = np.max(sp[:,1]) - np.min(sp[:,1])
    d = (d0**2 + d1**2)**0.5
    s = Sampler(ScoredKernel(d/20, space, scores, alpha, gamma), space, k)
    s.warmStart()
    return s
...
def cond_setup_sampler(space, scores, cond_ids, k, alpha, gamma):
    ...
    d1 = np.max(sp[:,1]) - np.min(sp[:,1])
    d = (d0**2 + d1**2)**0.5
    s = Sampler(ConditionedScoredKernel(d/20, space, scores, cond_ids, alpha, gamma), space, k)
    s.warmStart()
    return s
